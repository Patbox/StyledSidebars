        ...
        public record Serializer(HolderLookup.Provider lookup) implements JsonSerializer<Line>, JsonDeserializer<Line> {
            ...
            public JsonElement serialize(Line src, Type typeOfSrc, JsonSerializationContext context) {
                if (src.require == null && src.values.size() == 1) {
                    if (src.values.get(0).getSecond().isEmpty()) {
                        return new JsonPrimitive(src.values.get(0).getFirst());
                    } else {
                        var arr = new JsonArray();
                        arr.add(src.values.get(0).getFirst());
                        arr.add(src.values.get(0).getSecond());
                        return arr;
                    }
                } else {
                    var obj = new JsonObject();
                    if (src.values.stream().allMatch(x -> x.getSecond().isEmpty())) {
                        if (src.values.size() == 1) {
                            obj.add("values", new JsonPrimitive(src.values.get(0).getFirst()));
                        } else {
                        ...
                        for (var l : src.values) {
                            ar.addProperty(l.getFirst(), l.getSecond());
                        }
                        obj.add("values", ar);
                    }
